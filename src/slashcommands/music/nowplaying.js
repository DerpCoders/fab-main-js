...
module.exports = {
   ...
   * @param {Array} serverQueue.songs 
   * @returns 
   */
  async execute(interaction, serverQueue) {
    if (!interaction.member.voice.channel) return await interaction.followUp({content: '❌ **You need to be in a voice channel!**', flags: MessageFlags.Ephemeral });
    if (!interaction.guild.members.me.voice.channel) return;
    if (interaction.guild.members.me.voice.channel.id != interaction.member.voice.channel.id) return await interaction.followUp({content: '❌ **You need to be in the same voice channel!**'});
    if (!serverQueue) return await interaction.followUp({content: 'There\'s nothing in queue!'});
    let nextsong;
