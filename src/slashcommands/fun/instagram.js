...
module.exports = {
    ...
    async execute(interaction, username) {
        let mycookie = process.env.instacookie
        let ig = new igApi(mycookie);
        try {
            const res = await ig.fetchUser(username);
            ...
            await interaction.followUp({ embeds: [ebed] })
        } catch (err) {
            return await interaction.followUp({ content: "<:disagree:1021025405953654804> **User not found!**" })
        }
    }
}