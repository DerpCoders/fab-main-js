...
module.exports = {
    ...
    else if(killUser.id === '759762948016177195') return message.channel.send('Hahaha, you can\'t do that with me.')
   
    else{
            const aEmbed = new Discord.MessageEmbed()
            .setColor('RANDOM')
            .setAuthor(`${message.author.username} killed ${message.guild.member(killUser).displayName}..... `+ title , message.author.displayAvatarURL({size: 2048, dynamic: true}))
            .setImage(URL=url1)
            .setFooter(`${origin}  ||  RIP ${message.guild.member(killUser).displayName}`)
