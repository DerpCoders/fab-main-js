...
module.exports = { name: 'hug', async execute(message, args) {
    try{
...
}else if(mem.user.bot) return message.channel.send('No, not with bots!')
else if(mem.id === '759762948016177195') return message.channel.send('Nope');
else{
        const embed = new Discord.MessageEmbed()
        .setAuthor(`${message.author.username} hugged ${mem.user.username} :D`, message.author.displayAvatarURL({dynamic: true}))
        .setColor("RANDOM")
        .setFooter(`${mem.user.username} || Source: Anime Nekos`, mem.user.displayAvatarURL({dynamic: true}))
        .setImage(image.url);
