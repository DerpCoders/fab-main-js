...
module.exports = {
  ...
  execute(message) {
    ...
    message.author.send({ embeds: [exampleEmbed] }) &&
      message.author.send({ content: "Join if you still need help - discord.gg/astromk" })
        .catch((err) => {
          return (
            message.channel.send({ content: "**You have DMs turned off!**" }) &&
            console.log(err)
          );
        });
    message.react("🇩");
