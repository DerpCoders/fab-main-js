...
module.exports = {
    name: 'playlist',
    async execute(message, args, play, client, serverQueue, queue) {
        let data = await playSchema.findOne({ userID: message.author.id });
        if (!data) return message.channel.send({ content: "You don't have any playlist.\nUse `createplaylist <name>` to create one now!" });
        ...
        let plMsg = await message.channel.send({ content: '🎵 **Select a playlist from below-**', components: [row] })
        ...
        collector.on('collect', async (interaction) => {
            ...
            const embed = new Discord.EmbedBuilder()
                ...
                .setColor('Random')
                .setFooter({text: `${message.author.globalName}'s playlist`, iconURL: message.author.displayAvatarURL()})
            const selectionMsg = await message.reply({ embeds: [embed], components: [buttons] });
            ...
            buttonCollector.on('collect', async (interaction) => {
                ...
                if (interaction.customId === 'play') {
                    ...
                    if (!message.member.voice.channel) return await interaction.followUp({ content: `You need to be in a voice channel!` });
                    const perms = message.member.voice.channel.permissionsFor(message.client.user);
                    if (!perms.has(Discord.PermissionFlagsBits.Connect)) return await interaction.followUp({ content: `❌ **I don\'t have permission to join ${message.member.voice.channel.name}!**` });
                ...
                if (interaction.customId === 'delete') {
                    ...
                    if (data.playlists.length === 1) {
                        await playSchema.deleteOne({userID: message.author.id});
