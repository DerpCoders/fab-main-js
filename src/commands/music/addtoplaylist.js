...
module.exports = {
    ...
    async execute(message, args) {
        try {
            const data = await playSchema.findOne({ userID: message.author.id });
            ...
            playCollector.on('collect', async (interaction) => {
                ...
                const songCollector = message.channel.createMessageCollector({
                ...
                songCollector.on('collect', async (msg) => {
                    ...
                    if (msg.content.toLowerCase().includes('spotify.com/playlist')) {
                        let msgg = await message.channel.send("*Fetching data from spotify...*");
                        try {
                            const dat = await getData(msg.content);
                            if (!dat || typeof dat !== "object" || JSON.stringify(dat).includes("<!DOCTYPE html")) {
                                return message.channel.send(":x: This Spotify playlist is either private, invalid, or requires authentication.");
                            }
