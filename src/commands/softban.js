module.exports = { name: 'softban', description: 'softban', execute(message, args){
    try{

if(!message.member.hasPermission('MANAGE_ROLES')) return message.channel.send('❌**You don\'t have `MANAGE ROLES` permission!**');
...
if(banuser.hasPermission('ADMINISTRATOR')) return message.channel.send('❌ **That user is an Admin!**');
if(banuser.roles.cache.has(banRole)) return message.channel.send('That member is already banned.');
