  ...
  if (message.mentions.users.last()) {
    ...
    leveling
      ...
      .then(async (users) => {
        ...
        if (users[3])
          var forthplace = await client.users.fetch(users[3].userid);
        if (users[4])
          var fifthplace = await client.users.fetch(users[4].userid);
        if (users[5])
          var sixthplace = await client.users.fetch(users[5].userid);
        if (users[6])
          var seventhplace = await client.users.fetch(users[6].userid);
        if (users[7])
          var eightplace = await client.users.fetch(users[7].userid);
        if (users[8])
          var ninthplace = await client.users.fetch(users[8].userid);
        if (users[9])
        ...
        message.channel.send(
          new Discord.MessageEmbed()
            ...
            .setColor("#00FF00")
            .setDescription(
              `**1. ${(firstplace && firstplace) || "Nobody Yet"}**\nLevel: ${
                (users[0] && users[0].level) || "None"
              }\nXP: ${(users[0] && users[0].xp) || "None"}\n\n**2. ${
                (secondplace && secondplace) || "Nobody Yet"
              }**\nLevel: ${(users[1] && users[1].level) || "None"}\nXP: ${
                (users[1] && users[1].xp) || "None"
              }\n\n**3. ${
                (thirdplace && thirdplace) || "Nobody Yet"
              }**\nLevel: ${(users[2] && users[2].level) || "None"}\nXP: ${
                (users[2] && users[2].xp) || "None"
              }\n\n**4. ${
                (forthplace && forthplace) || "Nobody Yet"
              }**\nLevel: ${(users[3] && users[3].level) || "None"}\nXP: ${
                (users[3] && users[3].xp) || "None"
              }\n\n**5. ${
                (fifthplace && fifthplace) || "Nobody Yet"
              }**\nLevel: ${(users[4] && users[4].level) || "None"}\nXP: ${
                (users[4] && users[4].xp) || "None"
              }\n\n**6. ${
                (sixthplace && sixthplace) || "Nobody Yet"
              }**\nLevel: ${(users[5] && users[5].level) || "None"}\nXP: ${
                (users[5] && users[5].xp) || "None"
              }\n\n**7. ${
                (seventhplace && seventhplace) || "Nobody Yet"
              }**\nLevel: ${(users[6] && users[6].level) || "None"}\nXP: ${
                (users[6] && users[6].xp) || "None"
              }\n\n**8. ${
                (eightplace && eightplace) || "Nobody Yet"
              }**\nLevel: ${(users[7] && users[7].level) || "None"}\nXP: ${
                (users[7] && users[7].xp) || "None"
              }\n\n**9. ${
                (ninthplace && ninthplace) || "Nobody Yet"
              }**\nLevel: ${(users[8] && users[8].level) || "None"}\nXP: ${
                (users[8] && users[8].xp) || "None"
              }\n\n**10. ${
                (tenthplace && tenthplace) || "Nobody Yet"
              }**\nLevel: ${(users[9] && users[9].level) || "None"}\nXP: ${
                (users[9] && users[9].xp) || "None"
              }`
            )
        );
      });
...
// PROFILE ✓
  ...
  if (levelinfo.level > 0) {
    const rank = new canvacord.Rank()
      ...
      .setBackground(
        "IMAGE",
        "https://cdn.discordapp.com/attachments/760049923995074560/766916023755997244/sky_light_by_bisbiswas_de3r7zn-250t.jpg"
      )
    ...
    message.channel.send(
      new Discord.MessageEmbed()
        ...
        .setColor("#00ff00")
        .setDescription(
          `You are Level 0 LOL!! Send some messages and try again.`
        )
    );
  }
}
